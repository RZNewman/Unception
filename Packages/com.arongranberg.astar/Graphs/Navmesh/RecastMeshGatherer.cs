...
namespace Pathfinding.Graphs.Navmesh {
	...
	public class RecastMeshGatherer {
		...
		bool AddNewMesh (Renderer renderer, Mesh mesh, int submeshStart, int submeshCount, out GatheredMesh gatheredMesh) {
				...
				for (int i = 0; i < meshFilters.Length; i++) {
					...
					if (rend.isPartOfStaticBatch) {
						...
						if (rend.bounds.Intersects(bounds)) {
							...
							if (AddNewMesh(rend, filter.sharedMesh, rend is MeshRenderer mrend ? mrend.subMeshStartIndex : 0, dummyMaterials.Count, out var gatheredMesh)) {
		...
		void AddRecastMeshObj (RecastMeshObj recastMeshObj) {
			...
			if (filter != null) {
				...
				if (filter.TryGetComponent<MeshRenderer>(out var rend) && mesh != null) {
					if (AddNewMesh(rend, filter.sharedMesh, rend.subMeshStartIndex, rend.sharedMaterials.Length, out var gatheredMesh)) {
			...
			for (int i = 0; i < treeInstances.Length; i++) {
				...
				if (!cachedTreePrefabs.TryGetValue(prot.prefab, out TreeInfo treeInfo)) {
					...
					for (int j = 0; j < colliders.Count; j++) {
						...
						if (GetColliderMesh(collider, rootMatrixInv * collider.transform.localToWorldMatrix) is GatheredMesh mesh) {
